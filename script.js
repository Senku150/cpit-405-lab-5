...
//console.log(data[1].name) I can use this or use forEach

document.addEventListener("DOMContentLoaded", () => {
    //creating the tables and the filters -----------------------------------------------------------------------------------------------------------------
    ...
    p.forEach(pe => {
        ...
        <td>${pe.assists}</td>
        `
        tbody.appendChild(row); //also append will works 
        //the diffrence between append and appendChild are the flexabality which append can be used with more than 1 elemnt
        //However, append doens't support the old browsers so it's safer to use appendChild
    })
    let htmlForFilter= `<option value="all">All</option>`
    teams.forEach(team=>{
    htmlForFilter +=(`<option value="${team}">${team}</option>`)
    })
    team_filter.innerHTML=htmlForFilter;
//end of creating the tables and the filters -----------------------------------------------------------------------------------------------------------------
    function handleSortingWithEvent(e){ //TODO
    ...
    function handleSearch() { //if search happens re build the table to show the new result (there might be a better way to do it)
    ...
    //two steps to make intreacvtive website
    let searchE = document.getElementById("search") //first we grap the element from the html code
    searchE.addEventListener("keyup", handleSearch) //then we add an event listener to do somthing when somthing happens example: mouse click (`event name` , fucniton that will be triggerd)
    //in our example the event is 'keyup' 
    team_filter.addEventListener("change",handleSearch) //also we need trigger this event when we change the value of the select elemnt
...
function changeMode(){
    ...
    sorting.addEventListener("click",handleSortingWithEvent)
